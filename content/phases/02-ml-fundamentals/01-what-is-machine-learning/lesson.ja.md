...
```mermaid
flowchart TD
    A["データはあるか？"] -->|No| B["まずデータを集めるかルールを使う"]
    A -->|Yes| C["ルールを明示的に書けるか？"]
    C -->|"Yes、かつシンプル"| D["ルールを使う。MLはスキップ。"]
    C -->|"No、または複雑すぎる"| E["エラーのコストは許容できるか？"]
    E -->|"No、保証された正確さが必要"| F["決定論的手法を使う"]
    E -->|Yes| G["説明可能性が必要か？"]
    G -->|"Yes、厳格に"| H["解釈可能なモデルのみ使う"]
    G -->|"No、または部分的に"| I["MLを使う"]
    I --> J["十分なラベル付きデータがあるか？"]
    J -->|Yes| K["教師あり学習"]
    J -->|"一部のラベル"| L["半教師あり学習"]
