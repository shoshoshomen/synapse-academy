...
```mermaid
flowchart LR
  T[トークン] --> E[トークン+位置<br/>エンベディング]
  E --> B[トランスフォーマーボディ<br/>Nブロック]
  B --> H1[旧: LMヘッド<br/>語彙プロジェクション]
  B --> H2[新: 分類ヘッド<br/>2ロジットへの線形]
  H2 --> L[クロスエントロピー損失<br/>対ラベル]
```
...
```mermaid
flowchart TD
  Toks[(SMSフィクスチャ<br/>800ラベル付き)] --> Tok[ByteTokenizer<br/>語彙260]
  Tok --> DS[ClassificationDataset<br/>パッド+マスク]
  DS --> DL[DataLoader<br/>バッチ化]
  DL --> M[分類器<br/>ボディ + 平均プール + ヘッド]
  M --> L[クロスエントロピー損失]
  L --> O[Adamオプティマイザー]
  O -->|ヘッドのみ| M
  O -->|フルFT| M
  M --> E[評価器<br/>適合率 / 再現率 / F1]
```
